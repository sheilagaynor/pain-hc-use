...
odds.summary[1:15,7] <- formatC(round(as.numeric(odds.summary[1:15,7]), 3), digits =3,format = "f")
odds.summary[2,7] <- "<0.001"
...
png("StartingForestPlot.png", width=1550, height=800)
...
segments(-4.15, 10.2, 5.27, 10.2, lwd=2.5)
segments(-4.15, 0, 5.27, 0, lwd=2.5)
...
text(x = 2.95, c(9:8, 6:4), labels = annotext, adj=c(0,0), cex=2)
text(x = 2.85, 11, labels = "Odds Ratio (95% CI)", adj=c(0,0), cex=2, font=2)
...
text(x=4.65, c(9:8, 6:4), labels=odds.summary[,7],cex=2, adj=c(0,0))
text(x=4.65, 11, labels="P-Value", cex=2, adj = c(0,0), font=2)
...
for (i in 0:1) {
  arrows(i, 2.6, i+1, 2.6, angle=90, code=3, length=0.15, lwd=2)}

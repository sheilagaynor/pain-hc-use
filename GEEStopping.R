...
##Next getting cases that do STOP HC during study
qhu.complete$stopHCuse <- FALSE
for (i in 1:length(unique(qhu.complete$respid))) {
  id <- unique(qhu.complete$respid)[i]
  obs <- which(qhu.complete$respid==id)
  numqhu <- length(obs)
  if (numqhu>1) {
    for (j in 2:numqhu) { qhu.complete$stopHCuse[obs[j]] <- qhu.complete$QHU_Hormones[obs[j]]==2 & qhu.complete$QHU_Hormones[obs[j-1]]==1}}}

#Subset the data
stoppers <- qhu.complete[qhu.complete$stopHCuse==TRUE,]
ID_stopping <- unique(stoppers$respid)
qhu.subset <- qhu.complete[qhu.complete$respid %in% ID_stopping, ]

##Removing any time off the beginning from before they start using it
qhu.subset$nouse <- FALSE
for (ijk in 1:length(table(qhu.subset$respid))) {
  id <- unique(qhu.subset$respid)[ijk]
  obs <- which(qhu.subset$respid==id)
  numqhu <- length(obs)
  if (numqhu>1) {
    for (ijkk in 1:numqhu) {
      if (qhu.subset$QHU_Hormones[obs[ijkk]]==1 & qhu.subset$nouse[obs[ijkk]]==FALSE)  #Start at using and take all obs after it
      {qhu.subset$nouse[obs[ijkk:numqhu]] <- TRUE}}}}
qhu.subset <- qhu.subset[qhu.subset$nouse==TRUE,]

##Removing any time off the end when they start using it again
qhu.subset$start <- FALSE
for (ij in 1:length(unique(qhu.subset$respid))) {
  id <- unique(qhu.subset$respid)[ij]
  obs <- which(qhu.subset$respid==id)
  numqhu <- length(obs)
  if (numqhu<2) {qhu.subset$start[obs] <- TRUE} #delete if only one obs per respid
  if (numqhu>1) {
    for (jj in 2:numqhu) {
      if (qhu.subset$QHU_Hormones[obs[jj]]==1 & qhu.subset$QHU_Hormones[obs[jj-1]]==2)
      {qhu.subset$start[obs[jj:numqhu]] <- TRUE}}}}
qhu.usable <- qhu.subset[qhu.subset$start==FALSE,]
...
qhu.usable$HCdummy <- 1-qhu.usable$HCdummy
...
write.csv(summary, file="~/Dropbox/Sheila/OCP2018/Output/odds.StopGEE.v2.csv")

...
#Remove those with pre-existing pain
respExclude <- unique(c(qhucomplete.case$respid[which(qhucomplete.case$GYN_AmtPainPeriodsPst3Months==3)]),
                 (qhucomplete.case$respid[which(qhucomplete.case$GYN_AmtPainPeriodsPst3Months==4)]),
                 (qhucomplete.case$respid[which(qhucomplete.case$GYN_PrimaryReasonUsingHC==3)]),
                 (qhucomplete.case$respid[which(qhucomplete.case$GYN_ReasUseHCPainfulPeriods==1)]),
                 (qhucomplete.case$respid[which(qhucomplete.case$GYN_ReasUseHCPelvicPainEndomet==1)]))
qhucomplete.case <- qhucomplete.case[ ! qhucomplete.case$respid %in% respExclude, ]
...
write.csv(summary, file="~/Dropbox/Sheila/OCP2018/Output/odds.RestrictedGEE.v2.csv")
